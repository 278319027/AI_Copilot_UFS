...
- 生成代码
- 生成测试建议
- 生成 proposal.md ← 需求阶段产出（模板见 spec_workflow.md §3）
...
1. 需求理解摘要
2. 影响范围分析
3. 设计方案 → 输出 `design.md`（模板见 spec_workflow.md §1）
4. 编码清单 → 输出 `tasks.md`（模板见 spec_workflow.md §2）
5. 代码实现
6. 测试建议
## 标准流程

0. 理解需求 → 产出 proposal.md（声明意图、范围、验收标准）
1. **查询 CodeGraph** ← proposal.md 作为范围参考，深度照旧
2. 定位相关代码
3. 分析依赖关系
4. 输出设计方案 → 产出 `design.md`（含 CodeGraph 查询结果、待确认清单）
5. 生成代码 → 按 `tasks.md` 粒度（200-500 行/任务）执行
6. 生成测试建议
## CodeGraph 查询步骤
