...
hdr "1/17" "OpenSpec specs validation"
...
hdr "3/17" "Memory rules (5 files present)"
...
hdr "4/17" "scripts/deploy_tools.sh syntax"
...
# [5/17] Tools on PATH
hdr "5/17" "Essential tools on PATH"
MT=()
...
# [6/17] Memory rules format (non-empty + has headers)
hdr "6/17" "Memory rules format (5 files)"
BAD=0
for r in architecture.md concurrency_rules.md coding_style.md design_rules.md testing_rules.md; do
    f="$MEM/$r"
...
# [7/17] openspec/config.yaml non-empty
hdr "7/17" "openspec/config.yaml"
...
# [8/17] opencode.json 完整性与 JSON 有效性
hdr "8/17" "opencode.json schema (mcp + codegraph)"
...
# [9/17] FEMU_ROOT — 单一真相源：FEMU_ROOT env var 与 opencode.json codegraph --path 同步
hdr "9/17" "FEMU_ROOT (CodeGraph MCP target path)"
if [ -d "$FEMU_BASE" ]; then
...
hdr "10/17" "Spec files (3 capabilities, non-empty)"
EMPTY_SPECS=0
...
# [11/17] Skill directories: 5 openspec-* (硬) + 1 openspec-workflow (硬) + N superpowers-* (软报告) + 1 sd-firmware-copilot (硬)
hdr "11/17" "Skill directories (5 openspec-* + 1 openspec-workflow + N superpowers-* + 1 sd-firmware-copilot; N 不强制)"
SKILL_BAD=0
SP_OS_PHASE_COUNT=0
...
# [12/17] FEMU path should not contain mis-generated .opencode/ (zsf is the canonical .opencode owner)
hdr "12/17" "FEMU path .opencode/ cleanliness (only zsf should own .opencode/)"
...
# [13/17] scripts/check_change.sh 工具存在 + 可执行
hdr "13/17" "scripts/check_change.sh 工具"
...
# [14/17] sd-firmware-copilot SKILL.md 关键章节齐全
hdr "14/17" "sd-firmware-copilot SKILL.md 关键章节"
...
# [15/17] anti_patterns.md 存在性（非阻塞，信息提示）
hdr "15/17" "anti_patterns.md 存在性"
...
# [16/17] spec 符号存在性检查（可选，需要目标代码库）
hdr "16/17" "spec symbols existence"
...
# Summary
printf "\n==============================================\n"
printf "  Summary: %d/17 checks passed\n" "$P"
printf "==============================================\n"
